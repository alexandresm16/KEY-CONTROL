from django.contrib import messages
from django.contrib.messages.views import SuccessMessageMixin
from django.core.paginator import Paginator
from django.urls import reverse_lazy
from django.utils import timezone
from django.views.generic import ListView, CreateView, UpdateView, DeleteView, DetailView
from retiradabloco.forms import RetiradaBlocoModelForm
from retiradabloco.models import RetiradaBloco


# Create your views here.

class RetiradaBlocoView(ListView):
    model = RetiradaBloco
    template_name = 'retirada_bloco.html'

    def get_queryset(self):
        buscar = self.request.GET.get('buscar')
        qs = super(RetiradaBlocoView, self).get_queryset()
        for retirada in qs:
            if retirada.status == 'Retirada':
                chave = retirada.chave
                chave.disponibilidade = 'N'
                chave.save()
                if retirada.devolucao < timezone.now():
                    retirada.status = 'Expirada'
                    retirada.chave.disponibilidade = 'N'
                    retirada.save()
        if buscar:
            qs = qs.filter(chave__codigo__icontains=buscar)

        if qs.count() > 0:
            paginator = Paginator(qs, 10)
            listagem = paginator.get_page(self.request.GET.get('page'))
            return listagem
        else:
            return messages.info(self.request, "Não existem chaves cadastradas!")


class RetiradaBlocoAddView(SuccessMessageMixin, CreateView):
    form_class = RetiradaBlocoModelForm
    model = RetiradaBloco
    template_name = 'retirada_chave_bloco.html'
    success_url = reverse_lazy('retirada_bloco')
    success_message = 'Retirda da Chave realizada com sucesso!'


class RetiradaBlocoUpDateView(SuccessMessageMixin, UpdateView):
    form_class = RetiradaBlocoModelForm
    model = RetiradaBloco
    template_name = 'retirada_chave_bloco.html'
    success_url = reverse_lazy('retirada_bloco')
    success_message = 'Retirda da Chave alterada com sucesso!'


class RetiradaBlocoDeleteView(SuccessMessageMixin, DeleteView):
    model = RetiradaBloco
    template_name = 'apagar_retirada_bloco.html'
    success_url = reverse_lazy('retirada_bloco')
    success_message = 'Retirada da chave do bloco excluida com sucesso!'


class RetiradaBlocoDetailView(SuccessMessageMixin, DetailView):
    model = RetiradaBloco
    template_name = 'detail_retirada_bloco.html'
    success_url = reverse_lazy('retirada_bloco')
    success_message = 'Chave devolvida com sucesso!'

    def get_object(self, queryset=None):
        retirada = RetiradaBloco.objects.get(pk=self.kwargs.get('pk'))
        retirada.status = 'Devolvida'
        chave = retirada.chave
        chave.disponibilidade = 'S'
        chave.save()
        retirada.save()
        return retirada
